...
import type { IncidentRecord, KpiSnapshot, Policy } from '@/lib/api';
...
type DashboardProps = {
    ...
    onTriggerIncident: () => Promise<void>;
};
...
export const PyramidDashboard = (props: DashboardProps) => {
    const { latestIncident, latestKpi, policy, apiOnline, reasoningModel, loading, loadError, onTriggerIncident } = props;
    ...
    const dynamicLogs = useMemo(() => {
        if (!latestIncident) {
            return [{ msg: "Awaiting live incident stream...", type: 'info' as LogType, time: new Date().toLocaleTimeString() }];
        }

        const policyMessage = latestIncident.learning.policyUpdated
            ? "LEARNING LOOP: POLICY UPDATED FROM INCIDENT"
            : "LEARNING LOOP: POLICY CHECK COMPLETE";
        const reasoning = latestIncident.orchestrator.reasoning;
        const reasoningMsg = reasoning?.fallback
            ? `NOVA FALLBACK ACTIVE: ${reasoning.fallbackReason || 'bedrock unavailable'}`
            : `REASONING PROVIDER: ${(reasoning?.provider || 'sim').toUpperCase()} / ${reasoning?.modelId || 'local-heuristic-simulator'}`;

        return [
            { msg: `INCIDENT ${latestIncident.id.toUpperCase()} DETECTED (${latestIncident.orchestrator.priority})`, type: 'critical' as LogType, time: new Date(latestIncident.startedAt).toLocaleTimeString() },
            { msg: `SEVERITY SCORE: ${latestIncident.orchestrator.severity}/10`, type: 'warning' as LogType, time: new Date(latestIncident.completedAt).toLocaleTimeString() },
            { msg: `ORCHESTRATOR ACTIVATED: ${latestIncident.orchestrator.activatedDomains.join(', ').toUpperCase()}`, type: 'info' as LogType, time: new Date(latestIncident.completedAt).toLocaleTimeString() },
            { msg: reasoningMsg, type: reasoning?.fallback ? 'warning' as LogType : 'info' as LogType, time: new Date(latestIncident.completedAt).toLocaleTimeString() },
            { msg: `FUSION EXECUTED ${latestIncident.fusionDecision.executedCount} ACTIONS`, type: 'success' as LogType, time: new Date(latestIncident.completedAt).toLocaleTimeString() },
            { msg: policyMessage, type: latestIncident.learning.policyUpdated ? 'success' as LogType : 'info' as LogType, time: new Date(latestIncident.completedAt).toLocaleTimeString() },
        ];
    }, [latestIncident]);
    ...
    useEffect(() => {
        ...
        const severity: Severity = latestIncident.orchestrator.severity >= 8 ? 'critical' : latestIncident.orchestrator.severity >= 5 ? 'warning' : 'nominal';
        const nextStatus = latestIncident.learning.policyUpdated
            ? 'POLICY UPDATED - STANDBY'
            : `SEVERITY: ${latestIncident.orchestrator.severity}/10 - ${latestIncident.orchestrator.priority}`;

        setState(prev => ({
            ...prev,
            orders: `${latestIncident.input.demandSpikePercent}+`,
            stock: `${Math.max(100 - latestIncident.input.stockDepletionPercent, 0).toFixed(1)}%`,
            riders: latestIncident.input.ridersAvailable,
            severity,
            severityScore: latestIncident.orchestrator.severity,
            statusText: nextStatus,
            activeAgents: [...latestIncident.orchestrator.activatedDomains, 'workers', 'aggregators'],
            decisionReady: true,
            incidentResolved: true,
            logs: [...dynamicLogs, ...prev.logs].slice(0, 50)
        }));
    }, [dynamicLogs, latestIncident]);

    useEffect(() => {
        if (!loadError) return;
        setState(prev => ({
            ...prev,
            logs: [{ msg: `BACKEND ERROR: ${loadError}`, type: 'critical', time: new Date().toLocaleTimeString() }, ...prev.logs].slice(0, 50)
        }));
    }, [loadError]);
    ...
    const handleTrigger = async () => {
        setState(prev => ({
            ...
            decisionReady: false,
            incidentResolved: false,
            statusText: 'CALCULATING SEVERITY...',
            activeAgents: ['demand', 'inventory', 'logistics', 'pricing', 'risk'],
            logs: [{ msg: 'INCIDENT TRIGGER RECEIVED. ORCHESTRATOR STARTING...', type: 'warning', time: new Date().toLocaleTimeString() }, ...prev.logs].slice(0, 50)
        }));
            ...
            <div className="fixed top-0 left-0 w-full h-14 bg-black border-b border-[#1E1E1E] z-[50] flex items-center justify-between px-12">
                <div className="flex items-center gap-6">
                    ...
                    <div className="flex items-center gap-2">
                        <span className={cn("w-2 h-2 rounded-full animate-pulse", apiOnline ? "bg-[#10B981]" : "bg-red-500")} />
                        <span className={cn("text-[10px] font-bold tracking-widest uppercase pt-0.5", apiOnline ? "text-[#10B981]" : "text-red-500")}>
                            {apiOnline ? 'Live' : 'Offline'}
                        </span>
                    </div>
                    <span className="text-[10px] font-bold tracking-widest text-[#444444] uppercase pt-0.5 ml-4">
                        {latestIncident ? `${latestIncident.input.city} Ops ${latestIncident.input.zone}` : 'Mumbai Ops Zone 4'}
                    </span>
                </div>
                <div className="flex items-center gap-6 mr-56">
                    <button
                        ...
                        className="text-[10px] font-bold tracking-widest text-red-500 border border-red-500/30 px-4 py-1.5 rounded hover:bg-red-500 hover:text-white transition-all uppercase disabled:opacity-60 disabled:cursor-not-allowed"
                    >
                        {loading ? 'Running...' : 'Trigger Live Incident'}
                    </button>
                </div>
            </div>
                ...
                <div className="lg:col-span-1 flex flex-col gap-6">
                    <Card>
                        ...
                        <div className="space-y-6 mt-4">
                            {[
                                ...
                                { label: 'Avg Margin', value: latestIncident ? `${latestIncident.kpis.after.marginPercent}%` : '4.2%', icon: Tag },
                            ].map((item, i) => (
                    ...
                    <Card className="h-44">
                        <div className="flex items-center justify-between mb-4">
                            <h3 className="text-sm font-semibold text-[#888888] uppercase tracking-widest">Nova Reasoning Core</h3>
                            <div className="text-[11px] text-[#10B981] font-bold">
                                {Math.round((policy?.rules.confidenceThreshold ?? 0.94) * 100)}% CONFIDENCE
                            </div>
                        </div>
                ...
                <div className="lg:col-span-2 flex flex-col gap-8">
                    ...
                    <Card className="flex-none h-auto">
                        <div className="flex justify-between items-center mb-10">
                            <span className="text-[11px] uppercase font-bold text-[#888888] tracking-widest">{`Master Orchestrator (${reasoningModel})`}</span>
                        ...
                        <AnimatePresence>
                            {state.decisionReady ? (
                                ...
                                <button
                                    onClick={handleTrigger}
                                    disabled={loading || !apiOnline}
                                    className="w-full h-full bg-white text-black font-display text-3xl tracking-widest p-5 rounded-xl hover:bg-yellow-500 hover:text-black transition-all duration-300 active:scale-[0.98] flex items-center justify-center gap-6 group shadow-[0_0_30px_rgba(255,255,255,0.05)] disabled:opacity-60 disabled:cursor-not-allowed"
                                >
                                    <span className="pt-2">{loading ? 'Running Orchestrator...' : 'Trigger Incident: IPL Night Mumbai'}</span>
                ...
                <div className="lg:col-span-1 flex flex-col gap-6">
                    ...
                    <Card className="flex-none h-36">
                        <div className="flex items-center gap-3 text-[11px] font-bold text-[#888888] mb-6 uppercase tracking-widest">
                            ...
                            Uptime: {latestIncident ? new Date(latestIncident.completedAt).toLocaleTimeString() : '324:12:05'}
                        </div>
                ...
                {[
                    {
                        ...
                        value: latestIncident ? `${latestIncident.orchestrator.timeBudgetSec}s` : '9.2s',
                        trend: latestIncident ? 'AUTONOMOUS' : '-2.4s',
                        color: 'text-[#10B981]'
                    },
                    {
                        label: 'Cancellation Rate',
                        value: latestKpi ? `${latestKpi.cancellationsReducedPercent}%` : '1.24%',
                        trend: latestKpi ? 'REDUCED' : '-0.3%',
                        color: 'text-[#10B981]'
                    },
                    {
                        label: 'Margin Rescue',
                        value: latestIncident ? `INR ${latestIncident.kpis.impact.revenueImpactInr.toLocaleString()}` : '$4.2k',
                        trend: latestIncident ? `to ${latestIncident.kpis.after.marginPercent}%` : '+$1.1k',
                        color: 'text-[#10B981]'
                    },
                    {
                        label: 'AI Multi-Agent Load',
                        value: latestIncident ? `${latestIncident.fusionDecision.executedCount * 14}%` : '74%',
                        trend: apiOnline ? 'NOMINAL' : 'OFFLINE',
                        color: apiOnline ? 'text-[#888888]' : 'text-red-500'
                    },
                ].map((kpi, i) => (
